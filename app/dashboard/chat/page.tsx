  ...
  return (
    <div className="flex h-[calc(100vh-120px)] gap-4">
      <div className="w-48 space-y-2">
        <div className="text-jarvis-dim text-xs font-mono mb-3">SELECT AGENT</div>
        {AGENTS.map(agent => (
          <button
            key={agent.id}
      ...
      <div className="flex-1 flex flex-col bg-jarvis-surface border border-jarvis-border rounded-lg">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.length === 0 && (
            <div className="text-jarvis-dim text-xs font-mono text-center mt-16">
              Start a conversation with {selectedAgent.name}
            </div>
          )}
          {messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-2xl px-4 py-3 rounded text-xs font-mono ${
                msg.role === 'user'
          ...
          {loading && (
            <div className="flex justify-start">
              <div className="bg-jarvis-bg border border-jarvis-border px-4 py-3 rounded text-xs font-mono text-jarvis-dim animate-pulse">
        ...
        <div className="p-4 border-t border-jarvis-border flex gap-2">
          <input
            ...
            className="flex-1 bg-jarvis-bg border border-jarvis-border rounded px-3 py-2
                       text-jarvis-text font-mono text-xs focus:outline-none focus:border-jarvis-cyan"
          />
          <button
            onClick={sendMessage}
            disabled={loading}
            className="bg-jarvis-cyan text-jarvis-bg px-4 py-2 rounded font-mono text-xs
